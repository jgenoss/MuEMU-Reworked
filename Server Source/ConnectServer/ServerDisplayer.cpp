...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	// Fuentes mejoradas
	this->m_titleFont = CreateFont(18, 0, 0, 0, FW_BOLD, 0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_MODERN, "Segoe UI");
...
{
	char buff[256];
	wsprintf(buff, "Enhanced ConnectServer v2.0 - PREMIUM Edition (Queue: %d)", gSocketManager.GetQueueSize());
...
void CServerDisplayer::PaintTitleBar() // OK
{
	...
	char titleText[] = "CONNECTSERVER - PREMIUM EDITION";
	RECT textRect = rect;
...
void CServerDisplayer::PaintStatusSection() // OK
{
	...
	bool isActive = gServerList.CheckJoinServerState() != 0;
...
void CServerDisplayer::PaintInfoBar() // OK
{
	...
	// Clients (simulado)
	SetTextColor(hdc, RGB(52, 152, 219));
	TextOut(hdc, 180, rect.top + 8, "Clients:", 8);
	SetTextColor(hdc, RGB(255, 255, 255));
	TextOut(hdc, 225, rect.top + 8, "0", 1);
