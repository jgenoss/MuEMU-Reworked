...
void CServerDisplayer::PaintInfoBar() // OK
{
	...
	TextOut(hdc, 15, rect.top + 8, "Max Servers:", 8);
	SetTextColor(hdc, RGB(255, 255, 255));
	char serverCountStr[32];
	wsprintf(serverCountStr, "%d", MAX_SERVER);
	TextOut(hdc, 65, rect.top + 8, serverCountStr, strlen(serverCountStr));
	...
	// Servidores activos
	SetTextColor(hdc, RGB(52, 152, 219));
	TextOut(hdc, 180, rect.top + 8, "Active:", 7);
	SetTextColor(hdc, RGB(255, 255, 255));
	char activeStr[32];
	wsprintf(activeStr, "%d", this->GetActiveServerCount());
	TextOut(hdc, 220, rect.top + 8, activeStr, strlen(activeStr));
