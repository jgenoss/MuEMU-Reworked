...
void ProtocolCore(BYTE head, BYTE* lpMsg, int size, int aIndex, int encrypt, int serial) // OK
{
	...
	case 0xF3:
		...
		case 0xF1:
			...
			break;
		case 0xE5:
			CGEventScheduleRequestRecv(aIndex);
			break;
		}
		break;
	case 0xF6:
...
void CGSNSDataLogRecv(PMSG_SNS_DATA_LOG_RECV* lpMsg, int aIndex) // OK
{
	...
	LogAdd(LOG_WHITE, "[SNSDataLog][%s][%s] - (Code: %d/%d/%d)", lpObj->Account, lpObj->Name, lpMsg->code[0], lpMsg->code[1], lpMsg->code[2]);
...
void GCAnsRegCastleSiege(int iIndex, int iResult, LPSTR lpszGuildName)
{
	...
	{
		LogAdd(LOG_WHITE, "[CastleSiege] [%s][%s] Registered Castle Siege (GUILD:%s) - Result:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, iResult);
	}
#endif
}
...
void GCAnsGiveUpCastleSiege(int iIndex, int iResult, int bGiveUp, int iMarkCount, LPSTR lpszGuildName)
{
	...
	if (gObjIsConnected(iIndex))
	{
		LogAdd(LOG_WHITE, "[CastleSiege] [%s][%s] GiveUp Castle Siege (GUILD:%s) - Result:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, iResult);
	}
#endif
}
...
void CGReqNpcUpgrade(PMSG_REQ_NPCUPGRADE* lpMsg, int iIndex)
{
	...
	{
		LogAdd(LOG_WHITE, "[CastleSiege] CGReqNpcUpgrade() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus);
		return;
	}
	...
	case 283:
	{
		...
		if (bResult)
		{
			...
			{
				GCAnsNpcUpgrade(iIndex, 5, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
				LogAdd(LOG_WHITE, "[CastleSiege] CGReqNpcUpgrade() ERROR - UpType doesn't Exist [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
				return;
			}
			break;
			}
	...
	else
	{
		if (gObj[iIndex].Money < ((DWORD)iNEED_MONEY))
		{
			...
			LogAdd(LOG_WHITE, "[CastleSiege] CGReqNpcUpgrade() ERROR - Money is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Money:%d, Need Money:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue, iNXT_UPLVL, gObj[iIndex].Money, iNEED_MONEY);
		}
		else
		{
			...
			if (iGEMOFDEFEND_COUNT < iNEED_GEMOFDEFEND)
			{
				...
				LogAdd(LOG_WHITE, "[CastleSiege] CGReqNpcUpgrade() ERROR - Gem is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Gem:%d, Need Gem:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue, iNXT_UPLVL, iGEMOFDEFEND_COUNT, iNEED_GEMOFDEFEND);
			}
			else
			{
				...
				else
				{
					...
					LogAdd(LOG_WHITE, "[CastleSiege] [0xB2][0x07] CGReqNpcUpgrade() - Pay For Npc Upgrade (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)", lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, iNXT_UPVALUE);
					...
					LogAdd(LOG_WHITE, "[CastleSiege] CGReqNpcUpgrade() REQ OK - [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue, iNXT_UPLVL);
				}
			}
		}
	}
...
void CGReqTaxRateChange(PMSG_REQ_TAXRATECHANGE* lpMsg, int iIndex)
{
	...
	else
	{
		...
		if ((iTaxRate < 0) || (iTaxRate > iMaxTaxRate))
		{
			...
			LogAdd(LOG_WHITE, "[CastleSiege] CGReqTaxRateChange() ERROR - Tax Rate Out of Range [%s][%s], Guild:(%s)(%d), TaxType:%d, TaxRate:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus, lpMsg->btTaxType, iTaxRate);
		}
		else
		{
			GS_GDReqTaxRateChange(gMapServerManager.GetMapServerGroup(), iIndex, lpMsg->btTaxType, iTaxRate);
			LogAdd(LOG_WHITE, "[CastleSiege] CGReqTaxRateChange() REQ OK - [%s][%s], Guild:(%s)(%d), TaxType:%d, TaxRate:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus, lpMsg->btTaxType, iTaxRate);
		}
	}
#endif
}
...
void CGReqMoneyDrawOut(PMSG_REQ_MONEYDRAWOUT* lpMsg, int iIndex)
{
	...
	else
	{
		...
		if (iMoneyChange <= 0)
		{
			...
			LogAdd(LOG_WHITE, "[CastleSiege] CGReqMoneyDrawOut() ERROR - Req Money < 0 [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus, iMoneyChange, gObj[iIndex].Money);
		}
		else
		{
			...
			{
				GCAnsMoneyDrawOut(iIndex, 0, 0);
				LogAdd(LOG_WHITE, "[CastleSiege] CGReqMoneyDrawOut() ERROR - Castle Money is Low [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus, iMoneyChange, gObj[iIndex].Money);
			}
			else
			{
				...
				else
				{
					...
					LogAdd(LOG_WHITE, "[CastleSiege] CGReqMoneyDrawOut() REQ OK - [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus, iMoneyChange, gObj[iIndex].Money);
				}
			}
		}
	}
...
void CGReqCsGateOperate(PMSG_REQ_CSGATEOPERATE* lpMsg, int iIndex)
{
	...
	if (gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
	{
		...
		{
			LogAdd(LOG_WHITE, "[CastleSiege] CGReqCsGateOperate() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus);
			return;
		}
	}
	else
	...
	if (gCastleSiege.CheckCsGateAlive(iGateIndex) == TRUE)
	{
		...
		if (iOperateGate == 0)
		{
			...
			LogAdd(LOG_WHITE, "[CastleSiege] CGReqCsGateOperate() ERROR - Operate Gate Failed [%s][%s], Guild:(%s)(%d)", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus);
		}
		else
		{
			GCAnsCsGateOperate(iIndex, 1, iGateIndex, lpMsg->btOperate);
			gCastleSiege.SendCsGateStateViewPort(iGateIndex, lpMsg->btOperate);
			LogAdd(LOG_WHITE, "[CastleSiege] CGReqCsGateOperate() OK - [%s][%s], Guild:(%s)(%d), DOOR:(%d)(X:%d,Y:%d)(STATUE:%d)", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildNumber, iGateIndex, gObj[iGateIndex].X, gObj[iGateIndex].Y, gObj[iGateIndex].CsGateOpen);
		}
	}
	else
	{
		GCAnsCsGateOperate(iIndex, 2, -1, 0);
		LogAdd(LOG_WHITE, "[CastleSiege] CGReqCsGateOperate() ERROR - Gate Doesn't Exist [%s][%s], Guild:(%s)(%d)", gObj[iIndex].Account, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].GuildStatus);
	}
#endif
}
...
void GCAnsCsAccessCrownState(int iIndex, BYTE btCrownState)
{
	...
	if (btCrownState == 0)
	{
		...
		LogAdd(LOG_WHITE, "[CastleSiege] [Reg. Attempt] Accumulated Crown AccessTime : %d [%s](%s)(%s)", gObj[iIndex].AccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].Account, gObj[iIndex].Name);
	}

	if (btCrownState == 1)
	{
		LogAdd(LOG_WHITE, "[CastleSiege] [Reg. Success] Accumulated Crown AccessTime : %d [%s](%s)(%s)", gObj[iIndex].AccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].Account, gObj[iIndex].Name);
		gObj[iIndex].AccumulatedCrownAccessTime = 0;
	}
	...
	if (btCrownState == 2)
	{
		...
		LogAdd(LOG_WHITE, "[CastleSiege] [Reg. Fail] Accumulated Crown AccessTime : %d [%s](%s)(%s)", gObj[iIndex].AccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].Account, gObj[iIndex].Name);
	}
		...
		if (lpObj->Money >= ((DWORD)iEnterTaxMoney))
		{
			if (lpObj->Map == MAP_CASTLE_SIEGE)
			{
				if (gObjMoveGate(lpObj->Index, 95) == TRUE)
				{
					...
					LogAdd(LOG_WHITE, "[Castle HuntZone] [%s][%s] - Entrance TAX : %d - %d = %d", lpObj->Account, lpObj->Name, iOldMoney, iEnterTaxMoney, lpObj->Money);
				}
			}
		}
	}
...
{
	LogAdd(LOG_BLUE, "[EventSchedule] CGEventScheduleRequestRecv from aIndex=%d", aIndex);
	...
	GCEventScheduleInfoSend(aIndex);
}
...
void GCEventScheduleInfoSend(int aIndex)
{
	...
	PMSG_EVENT_SCHEDULE_INFO_SEND* pMsg = (PMSG_EVENT_SCHEDULE_INFO_SEND*)buffer;
	EVENT_SCHEDULE_DATA* pData = (EVENT_SCHEDULE_DATA*)(buffer + sizeof(PMSG_EVENT_SCHEDULE_INFO_SEND));

	int eventCount = 0;

	// Blood Castle (1 entrada consolidada - muestra el proximo)
	int bcMinTime = 999999;
	int bcBestLevel = -1;
	int bcState = 0;
	int bcUsers = 0;

	for (int i = 0; i < MAX_BC_LEVEL; i++)
	{
		int state = gBloodCastle.GetState(i);
		if (state != BC_STATE_BLANK)
		{
			int remainTime = gBloodCastle.GetRemainTime(i);
			if (remainTime < bcMinTime)
			{
				bcMinTime = remainTime;
				bcBestLevel = i;
				bcState = state;
				bcUsers = gBloodCastle.GetEnteredUserCount(i);
			}
		}
	}

	if (bcBestLevel >= 0 && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		pData[eventCount].EventType = EVENT_TYPE_BLOOD_CASTLE;
		pData[eventCount].EventIndex = (BYTE)bcBestLevel;
		pData[eventCount].State = (BYTE)bcState;
		pData[eventCount].RemainTime = (WORD)(bcMinTime > 65535 ? 65535 : bcMinTime);
		pData[eventCount].EnteredUsers = (BYTE)(bcUsers > 255 ? 255 : bcUsers);
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Blood Castle");
		eventCount++;
	}

	// Devil Square (1 entrada consolidada - muestra el proximo)
	int dsMinTime = 999999;
	int dsBestLevel = -1;
	int dsState = 0;
	int dsUsers = 0;

	for (int i = 0; i < MAX_DS_LEVEL; i++)
	{
		int state = gDevilSquare.GetState(i);
		if (state != DS_STATE_BLANK)
		{
			int remainTime = gDevilSquare.GetRemainTime(i);
			if (remainTime < dsMinTime)
			{
				dsMinTime = remainTime;
				dsBestLevel = i;
				dsState = state;
				dsUsers = gDevilSquare.GetEnteredUserCount(i);
			}
		}
	}

	if (dsBestLevel >= 0 && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		pData[eventCount].EventType = EVENT_TYPE_DEVIL_SQUARE;
		pData[eventCount].EventIndex = (BYTE)dsBestLevel;
		pData[eventCount].State = (BYTE)dsState;
		pData[eventCount].RemainTime = (WORD)(dsMinTime > 65535 ? 65535 : dsMinTime);
		pData[eventCount].EnteredUsers = (BYTE)(dsUsers > 255 ? 255 : dsUsers);
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Devil Square");
		eventCount++;
	}

	// Chaos Castle (1 entrada consolidada - muestra el proximo)
	int ccMinTime = 999999;
	int ccBestLevel = -1;
	int ccState = 0;
	int ccUsers = 0;

	for (int i = 0; i < MAX_CC_LEVEL; i++)
	{
		int state = gChaosCastle.GetState(i);
		if (state != CC_STATE_BLANK)
		{
			int remainTime = gChaosCastle.GetRemainTime(i);
			if (remainTime < ccMinTime)
			{
				ccMinTime = remainTime;
				ccBestLevel = i;
				ccState = state;
				ccUsers = gChaosCastle.GetEnteredUserCount(i);
			}
		}
	}

	if (ccBestLevel >= 0 && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		pData[eventCount].EventType = EVENT_TYPE_CHAOS_CASTLE;
		pData[eventCount].EventIndex = (BYTE)ccBestLevel;
		pData[eventCount].State = (BYTE)ccState;
		pData[eventCount].RemainTime = (WORD)(ccMinTime > 65535 ? 65535 : ccMinTime);
		pData[eventCount].EnteredUsers = (BYTE)(ccUsers > 255 ? 255 : ccUsers);
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Chaos Castle");
		eventCount++;
	}

	// Invasiones (mostrar las activas o proximas)
	for (int i = 0; i < MAX_INVASION && eventCount < MAX_EVENT_SCHEDULE_INFO; i++)
	{
		int state = gInvasionManager.GetState(i);
		if (state != INVASION_STATE_BLANK)
		{
			int remainTime = gInvasionManager.GetRemainTime(i);
			pData[eventCount].EventType = EVENT_TYPE_INVASION;
			pData[eventCount].EventIndex = (BYTE)i;
			pData[eventCount].State = (BYTE)state;
			pData[eventCount].RemainTime = (WORD)(remainTime > 65535 ? 65535 : (remainTime < 0 ? 0 : remainTime));
			pData[eventCount].EnteredUsers = 0;
			sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Invasion %d", i + 1);
			eventCount++;
		}
	}

	// Illusion Temple (1 entrada consolidada - muestra el proximo)
	int itMinTime = 999999;
	int itBestLevel = -1;
	int itState = 0;
	int itUsers = 0;

	for (int i = 0; i < MAX_IT_LEVEL; i++)
	{
		int state = gIllusionTemple.GetState(i);
		if (state != IT_STATE_BLANK)
		{
			int remainTime = gIllusionTemple.GetRemainTime(i);
			if (remainTime < itMinTime)
			{
				itMinTime = remainTime;
				itBestLevel = i;
				itState = state;
				itUsers = gIllusionTemple.GetEnteredUserCount(i);
			}
		}
	}

	if (itBestLevel >= 0 && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		pData[eventCount].EventType = EVENT_TYPE_ILLUSION_TEMPLE;
		pData[eventCount].EventIndex = (BYTE)itBestLevel;
		pData[eventCount].State = (BYTE)itState;
		pData[eventCount].RemainTime = (WORD)(itMinTime > 65535 ? 65535 : itMinTime);
		pData[eventCount].EnteredUsers = (BYTE)(itUsers > 255 ? 255 : itUsers);
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Illusion Temple");
		eventCount++;
	}

	// Crywolf
	int cwState = gCrywolf.GetCrywolfState();
	if (cwState != CRYWOLF_STATE_NONE && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		pData[eventCount].EventType = EVENT_TYPE_CRYWOLF;
		pData[eventCount].EventIndex = 0;
		pData[eventCount].State = (BYTE)cwState;
		pData[eventCount].RemainTime = 0; // Crywolf no tiene GetRemainTime publico
		pData[eventCount].EnteredUsers = 0;
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Crywolf");
		eventCount++;
	}

	// Kanturu
	if (gKanturu.GetKanturuEnable() != 0 && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		int kState = gKanturu.GetKanturuState();
		int kRemainTime = gKanturu.GetRemainTime();
		pData[eventCount].EventType = EVENT_TYPE_KANTURU;
		pData[eventCount].EventIndex = 0;
		pData[eventCount].State = (BYTE)kState;
		pData[eventCount].RemainTime = (WORD)(kRemainTime > 65535 ? 65535 : (kRemainTime < 0 ? 0 : kRemainTime));
		pData[eventCount].EnteredUsers = 0;
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Kanturu");
		eventCount++;
	}

	// Imperial Guardian
	int igState = gImperialGuardian.GetState();
	if (igState != IG_STATE_EMPTY && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		pData[eventCount].EventType = EVENT_TYPE_IMPERIAL_GUARDIAN;
		pData[eventCount].EventIndex = 0;
		pData[eventCount].State = (BYTE)igState;
		pData[eventCount].RemainTime = 0;
		pData[eventCount].EnteredUsers = (BYTE)gImperialGuardian.GetUserCount();
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Imperial Guardian");
		eventCount++;
	}

	// Double Goer (Doppelganger)
	int dgState = gDoubleGoer.GetState();
	if (dgState != DG_STATE_EMPTY && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		pData[eventCount].EventType = EVENT_TYPE_DOUBLE_GOER;
		pData[eventCount].EventIndex = 0;
		pData[eventCount].State = (BYTE)dgState;
		pData[eventCount].RemainTime = 0;
		pData[eventCount].EnteredUsers = (BYTE)gDoubleGoer.GetUserCount();
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Doppelganger");
		eventCount++;
	}

	// Raklion
	if (gRaklion.IsRaklionEnable() != 0 && eventCount < MAX_EVENT_SCHEDULE_INFO)
	{
		int rkState = gRaklion.GetRaklionState();
		pData[eventCount].EventType = EVENT_TYPE_RAKLION;
		pData[eventCount].EventIndex = 0;
		pData[eventCount].State = (BYTE)rkState;
		pData[eventCount].RemainTime = 0;
		pData[eventCount].EnteredUsers = 0;
		sprintf_s(pData[eventCount].EventName, sizeof(pData[eventCount].EventName), "Raklion");
		eventCount++;
	}

	pMsg->EventCount = (BYTE)eventCount;
	WORD size = sizeof(PMSG_EVENT_SCHEDULE_INFO_SEND) + (sizeof(EVENT_SCHEDULE_DATA) * eventCount);
	pMsg->header.set(0xF3, 0xE5, size);
	LogAdd(LOG_BLUE, "[EventSchedule] GCEventScheduleInfoSend to aIndex=%d EventCount=%d Size=%d", aIndex, eventCount, size);
