...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	// Colores modernos (mismo esquema que ConnectServer/DataServer)
...
{
	char buff[256];
	wsprintf(buff, "Enhanced JoinServer v2.0 - PREMIUM Edition (Queue: %d) (Accounts: %d/%d)",
		gSocketManager.GetQueueSize(), gAccountManager.GetAccountCount(), MAX_ACCOUNT);
...
void CServerDisplayer::PaintTitleBar() // OK
{
	...
	char titleText[] = "JOINSERVER - PREMIUM EDITION";
	RECT textRect = rect;
...
void CServerDisplayer::PaintStatusSection() // OK
{
	...
	// Lógica original del JoinServer para determinar estado
	int state = 0;
	...
	COLORREF indicatorColor = isActive ? RGB(255, 255, 255) : RGB(231, 76, 60);
	...
	// Indicador de cuentas (simple, sin formas complejas)
	int accountX = rect.right - 120;
	int accountY = rect.top + 25;

	// Fondo semi-transparente simulado
	RECT accountRect = { accountX, accountY, accountX + 100, accountY + 30 };
	HBRUSH accountBrush = CreateSolidBrush(RGB(0, 0, 0)); // Fondo oscuro
	FillRect(hdc, &accountRect, accountBrush);
	DeleteObject(accountBrush);

	// Borde
	HPEN accountPen = CreatePen(PS_SOLID, 1, RGB(255, 255, 255));
	SelectObject(hdc, accountPen);
	MoveToEx(hdc, accountRect.left, accountRect.top, NULL);
	LineTo(hdc, accountRect.right, accountRect.top);
	LineTo(hdc, accountRect.right, accountRect.bottom);
	LineTo(hdc, accountRect.left, accountRect.bottom);
	LineTo(hdc, accountRect.left, accountRect.top);
	DeleteObject(accountPen);

	// Texto de cuentas
	HFONT smallFont = CreateFont(10, 0, 0, 0, FW_BOLD, 0, 0, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_MODERN, "Segoe UI");
	HFONT oldSmallFont = (HFONT)SelectObject(hdc, smallFont);

	char accountText[64];
	wsprintf(accountText, "ACCOUNTS: %d/%d", gAccountManager.GetAccountCount(), MAX_ACCOUNT);
	TextOut(hdc, accountX + 5, accountY + 8, accountText, strlen(accountText));
...
void CServerDisplayer::PaintInfoBar() // OK
{
	...
	TextOut(hdc, 15, rect.top + 8, "Accounts:", 9);
	SetTextColor(hdc, RGB(255, 255, 255));
	char accountStr[32];
	wsprintf(accountStr, "%d", gAccountManager.GetAccountCount());
	TextOut(hdc, 70, rect.top + 8, accountStr, strlen(accountStr));

	// Queue
	SetTextColor(hdc, RGB(52, 152, 219));
	TextOut(hdc, 110, rect.top + 8, "Queue:", 6);
	SetTextColor(hdc, RGB(255, 255, 255));
	char queueStr[32];
	wsprintf(queueStr, "%d", gSocketManager.GetQueueSize());
	TextOut(hdc, 150, rect.top + 8, queueStr, strlen(queueStr));

	// Servidores conectados
	SetTextColor(hdc, RGB(52, 152, 219));
	TextOut(hdc, 190, rect.top + 8, "Servers:", 8);
	SetTextColor(hdc, RGB(255, 255, 255));
	char serverStr[32];
	wsprintf(serverStr, "%d", this->GetActiveServerCount());
	TextOut(hdc, 240, rect.top + 8, serverStr, strlen(serverStr));

	// Uptime
	SetTextColor(hdc, RGB(52, 152, 219));
	TextOut(hdc, 280, rect.top + 8, "Uptime:", 7);
	SetTextColor(hdc, RGB(255, 255, 255));
	...
	TextOut(hdc, 325, rect.top + 8, uptimeStr, strlen(uptimeStr));
...
int CServerDisplayer::GetActiveServerCount() // OK
{
	int activeCount = 0;
