	...
	try
	{
		while (true)
		{
			...
			if (type == 1)
			{
				...
				info.TypeShow = lpMemScript->GetAsNumber();
				...
				for (int i = 0; i < 15; i++)
				{
					...
					info.TextIndex1[i] = lpMemScript->GetAsNumber();
					info.TextColor1[i] = lpMemScript->GetAsNumber();
					info.TextIndex2[i] = lpMemScript->GetAsNumber();
					info.TextColor2[i] = lpMemScript->GetAsNumber();
					info.TextIndex3[i] = lpMemScript->GetAsNumber();
					info.TextColor3[i] = lpMemScript->GetAsNumber();
...
{
	if (info.ItemIndexMin < -1 || info.ItemIndexMax >= 7680 || CustomItemIndexCount >= MaxLine)
	{
		return;
	}
