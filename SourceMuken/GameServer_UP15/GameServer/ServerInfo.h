...
public:
	...
	int m_WingCustomMixRate[MAX_ACCOUNT_LEVEL];
	#endif

	#if(CUSTOM_MIX_WING_SEASON13)
	int m_WingSeason13MixRate[MAX_ACCOUNT_LEVEL];
	#endif

	#if(CUSTOM_MIX_WING_SEASON8)
	int m_WingSeason8MixRate[MAX_ACCOUNT_LEVEL];
	#endif

#if(CUSTOM_MIX_WING_VIP)
	int m_WingVipMixRate[MAX_ACCOUNT_LEVEL];
#endif
	
	#if(CUSTOM_MIX_PET_CAP2)
		int m_Muun2MixRate[MAX_ACCOUNT_LEVEL];
	#endif
	
	#if(CUSTOM_MIX_PET_CAP3)
		int m_Muun3MixRate[MAX_ACCOUNT_LEVEL];
	#endif
	
	#if(CUSTOM_MIX_PET_CAP4)
		int m_Muun4MixRate[MAX_ACCOUNT_LEVEL];
	#endif

#if(CUSTOM_MIX_PET_CAP5)
		int m_Muun5MixRate[MAX_ACCOUNT_LEVEL];
#endif

#if(CUSTOM_MIX_PET_CAP6)
		int m_Muun6MixRate[MAX_ACCOUNT_LEVEL];
#endif

#if(CUSTOM_MIX_PET_CAP7)
		int m_Muun7MixRate[MAX_ACCOUNT_LEVEL];
#endif

#if(CUSTOM_MIX_PET_CAP8)
		int m_Muun8MixRate[MAX_ACCOUNT_LEVEL];
#endif

#if(CUSTOM_MIX_PET_CAP9)
		int m_Muun9MixRate[MAX_ACCOUNT_LEVEL];
#endif

#if(CUSTOM_MIX_PET_CAP10)
		int m_Muun10MixRate[MAX_ACCOUNT_LEVEL];
#endif

#if(CUSTOM_MIX_PET_CAP11)
		int m_Muun11MixRate[MAX_ACCOUNT_LEVEL];
#endif
...
#if(CHONPHEDOILAP)
	int MoChucNangChonPhe;
#endif
#if(EVENT_MAPBAOTIEU)
	//===Event Bao Tieu
	char  EventBD_Mess[3][128];
	short BAOTIEU_NPC;
	short BAOTIEU_NPCX;
	short BAOTIEU_NPCY;
	short BAOTIEU_GATE;
