...
void cOfflineMode::GCOfflineModeRecv(PMSG_OFFLINEMODE_REQ* Data)
{
	...
	////console.Log("", "skill = %d,%d,%d", Data->SkillIndex[0], Data->SkillIndex[1], Data->SkillIndex[2]);
...
void cOfflineMode::DrawSkill(int Skill, int StartX)
{
	...
	if (this->SelectedSkill[Skill] == true)
	{
		...
		pDrawBarForm(PosX - 1, PosY - 1, 22, 31, 0.0f, 0);
		pGLSwitchBlend();
		pGLSwitch();
...
#pragma region Выбор скилла
	...
	else
	{
		...
		case SKILL_CHAIN_LIGHTNING:
			...
			DrawSkill = 1;
			break;
		case SKILL_DRAIN_LIFE: // SKILL_GHOST_PHANTOM
			ScaleX = 11 * 20;
			ScaleY = 3 * 28;
			SkillPattern = 0x7A53;
			DrawSkill = 1;
