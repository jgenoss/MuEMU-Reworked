...
void InsertNewHealthBar(WORD index, BYTE type, BYTE rate, BYTE rate2, BYTE bHasManyFlag) // OK
{
	for (int n = 0; n < MAX_MAIN_VIEWPORT; n++)
	{
		if (gNewHealthBar[n].index == 0xFFFF)
		{
			...
			gNewHealthBar[n].bHasManyFlag = bHasManyFlag;
			return;
		}
	}
}
	...
	if (HPMonsterName == 1)
	{
		for (int n = 0; n < MAX_MAIN_VIEWPORT; n++)
		{
			...
			else
			{
				...
				gCustomRankUser.DrawFormat(eColorHealth, PosX, PosY - 8, (int)LifeBarWidth, 3, "%s [%d%%]", name, PercentualMonster);
			}

			pSetBlend(true);

			float Tamanho = (ShieldPercent>0) ? Tamanho = 10.0f : Tamanho = 20.0f;
