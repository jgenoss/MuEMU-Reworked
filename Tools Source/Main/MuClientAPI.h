...
public:
	...
	inline uintptr_t RenderFrame() {
		return 0x0080F8E0;
	}

	inline int intRenderFrame(void* this_ptr) {
	...
	inline static void RenderImage(DWORD modelID, float x, float y, float w, float h) {
	...
	inline static int TextFormat(char* buffer, int arg2, int lineSize, LPCSTR text, int lineWidth, bool arg6, BYTE arg7) {
		...
		return reinterpret_cast<Fn>(0x00540880)(buffer, arg2, lineSize, text, lineWidth, arg6, arg7);
	}
	...
	inline static int _DrawText(LPVOID textThis, int x, int y, LPCSTR text, int nCount = 0, int nTabPos = 0, int* tabStops = nullptr, int tabOrigin = 0) {
	...
	inline static int LoadBitmap(char* folder, int code, int a3 = 0, int a4 = 0, int a5 = 0, int a6 = 0) {
